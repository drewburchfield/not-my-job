...
## Appendix: Multi-Harness Support

This skill is designed for Claude Code but can be adapted for Gemini CLI and Codex CLI users.

### Gemini CLI Extension Format

Gemini CLI has a full extension system. To adapt this skill:

**Directory Structure:**
```
braintrust-gemini/
├── gemini-extension.json    # Manifest
├── GEMINI.md               # Context file (copy of SKILL.md content)
└── commands/
    └── brain.toml          # Custom command
```
...
**commands/brain.toml:**
```toml
description = "Consult your AI braintrust for second opinions"
prompt = """Consult the braintrust (Claude + Codex) for: {{args}}

Use the braintrust patterns from your context to get second opinions."""
```
...
**Documentation:** [geminicli.com/docs/extensions](https://geminicli.com/docs/extensions/)

### Codex CLI Compatibility

Codex has no official plugin system but supports:

- **MCP servers:** `codex mcp add braintrust --command "..."`
- **Profile configs:** `~/.codex/config.toml`
- **Stdin prompts:** `codex exec - < SKILL.md`

**Shell wrapper example:**
```bash
#!/bin/bash
# braintrust-codex.sh
cat /path/to/SKILL.md | codex exec --json -
```

**Documentation:** Experimental MCP via `codex mcp --help`
